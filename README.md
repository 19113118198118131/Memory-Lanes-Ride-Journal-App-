...
| Coordinate | Create or join private group rides, RSVP, check in, share ride-day updates, report a ride, block a host and opt into live location or nearby Ride Mesh. |
...
- Group riding is currently best suited to small, trusted beta groups. Riders can report a ride or block its host, and reports require an operational review process before broader community release.
...
1. Apply [`supabase-beta-safety-account-deletion.sql`](supabase-beta-safety-account-deletion.sql) in the Supabase SQL Editor, then verify account deletion, reporting and host blocking with a non-owner test account.
2. Resolve the remaining operational items in [`docs/beta-release-preflight.md`](docs/beta-release-preflight.md).
3. Enable the Apple account for App Store Connect and authenticate `asc` using `asc auth login`.
4. Archive a signed Release build in Xcode.
5. Upload it to App Store Connect, add an internal tester group and attach concise What to Test notes.
